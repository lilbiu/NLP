...
def get_document_relevance_grader_chain():
    """获取文档相关性评估链 - 使用文本输出"""
    prompt = ChatPromptTemplate.from_messages([
        ("system", "你是一位信息相关性评估专家。请根据用户问题，判断下面提供的一组文档是否包含足够的相关信息来回答该问题。**只回复一个单词 true 或 false，不要任何解释。**"),
        ("human", "用户问题: {query}\n\n检索到的文档:\n{documents}")
    ])
...
def get_relevance_grader_chain():
    """获取相关性评估链 - 使用文本输出，避免 JSON 解析问题"""
    prompt = ChatPromptTemplate.from_messages([
        ("system", "你是一位信息相关性评估专家。请根据用户问题，判断提供的答案是否相关。**只回复一个单词 true 或 false，不要任何解释。**"),
        ("human", "问题: {query}\n答案: {response}")
    ])
    # 使用 StrOutputParser 获取纯文本，然后在节点中手动解析
    return prompt | llm | StrOutputParser()
