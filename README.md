# 基于大语言模型（LLM）和检索增强生成式（RAG）的医学知识智能问答系统
...
1.  **直接区块检索 (Direct Chunk Search)**：当用户查询一个**具体实体**（如疾病名称、具体症状、治疗方法）时，Agent会选择此策略。它将直接在最细粒度的“区块层”进行高精度搜索，确保最高的查全率。
